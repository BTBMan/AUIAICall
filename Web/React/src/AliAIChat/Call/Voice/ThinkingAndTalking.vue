...
function format(data: Uint8Array) {
  ...
  // 使用 Math.pow 进行指数缩放
  const exponent = 1.2
  const compressed = normalizedValue ** (1 / exponent)
...
export default defineComponent({
  ...
  setup() {
    ...
    const { agentState } = storeToRefs(useCallStore())
    ...
    onMounted(() => {
      ...
      if (controller.value.agentAudioElement) {
        createSourceNode(controller.value.agentAudioElement)
      }
      else {
        ...
        controller.value.on('AICallAgentAudioSubscribed', onAudioElement)
      }
      ...
      const setupAnimation = () => {
        ...
        if (agentState.value === AICallAgentState.Thinking) {
          ...
          if (!sourceNode.value || !audioContext)
            return
      ...
      onUnmounted(() => {
        ...
        controller.value.off('AICallAgentAudioSubscribed')
        audioContext?.close()
      })
    })
