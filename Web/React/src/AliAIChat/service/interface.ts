...
export function getAgentType(type: WorkflowType) {
  if (WorkflowTypeAgentTypeMap[type])
    return WorkflowTypeAgentTypeMap[type]
...
export function getWorkflowType(type?: AICallAgentType) {
  if (!type)
    return WorkflowType.VoiceChat
  if (AgentTypeWorkflowTypeMap[type])
    return AgentTypeWorkflowTypeMap[type]
  return WorkflowType.VoiceChat
}
