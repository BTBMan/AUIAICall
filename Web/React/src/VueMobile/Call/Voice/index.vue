  ...
  <div class="character voice">
    <div class="_box">
      <template v-if="avatarUrl">
        <div class="_agent-status-with-avatar">
          <div></div>
        </div>
        <div class="_avatar">
          <img :src="avatarUrl" />
      ...
      <div v-else class="_agent-status">
        <div
          class="animated-listening"
          :style="{ opacity: agentState === agentStates.Listening ? 1 : 0 }"
        >
          <voice-listening />
        </div>
        <div
          ...
          :style="{
            opacity:
              agentState === agentStates.Thinking ||
              agentState === agentStates.Speaking
                ? 1
                : 0,
          }"
        >
          <voice-thinking-and-talking />
        </div>
      </div>
    </div>
  </div>
...
<script lang="ts">
  ...
  import VoiceListening from './Listening.vue';
  import VoiceThinkingAndTalking from './ThinkingAndTalking.vue';
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const { agentState: storeAgentState, isSpeaking } = useCallModel();
      const agentState = ref(AICallAgentState.Listening);
      const showSpeaking = ref(false);
      const speakingTimer = ref<number | null>(null);
      const agentStates = AICallAgentState;

      const avatarUrl = computed(() => {
        return controller?.config.templateConfig?.avatarUrl;
      });

      // 先停止动画，再执行切换状态进行 transition
      watch(storeAgentState, (newState) => {
        setTimeout(() => {
      ...
      watch(isSpeaking, (newVal) => {
        if (newVal === showSpeaking.value) return;
        ...
        if (newVal) {
          ...
          showSpeaking.value = true;
        } else {
          speakingTimer.value = window.setTimeout(() => {
      ...
      onMounted(() => {
        // 初始化与storeAgentState同步
        agentState.value = storeAgentState.value;
      });
